...
if (isset($_POST['listReserve']) && $_POST['listReserve'] == "cancel") {
    ...
    if ($result1) {
        ...
        echo "window.location = 'approve.php?approve_cancel=approve_cancel'; ";
        echo "</script>";
    } else {
        $e = oci_error($result1 ? $resultinsertDetial : $result);
        oci_rollback($condb);
        echo "<script type='text/javascript'>";
        echo "window.location = 'approve.php?cancle_cancel_error=cancle_cancel_error'; ";
        echo "</script>";
    }
}

if (isset($_POST['listReserve']) && $_POST['listReserve'] == "approve"){
    $reserve_ID = $_POST["reservID"];
    $reserve_Status = "STA0000005";
    $query = "UPDATE reserveroom
    SET reservel_staID = :reservel_staID
    WHERE reserveID = :reserveID";
    $result = oci_parse($condb, $query);
    oci_bind_by_name($result, ':reservel_staID', $reserve_Status);
    oci_bind_by_name($result, ':reserveID', $reserve_ID);
